...
%Due to limitations in computational resources, we did not evaluate our models on any downstream tasks. Instead, we assessed their ability to predict the next tokens from the dataset. The simple MoE Transformer was selected as our baseline model for comparison. %to jest wlasnie language modelling
...
For MoE and MH-MoE models, the training loss is a sum of a primary cross-entropy loss related to the target task and an auxiliary load balancing loss (\cite{wu2024multihead}). The load balancing loss is computed as follows:
