...
% In a MoE model, each token is first routed to a particular expert (or experts), to be processed by it. In classic Moe the act of routing a token is based on choosing the maximum (or top k) weights produced by the router, which is not differentiable. Having to work around this limits the ways through which gradient can flow through the router. Additionally, Ensuring that all experts are equally utilized is very hard and requires carefully fine-tuning the loss function. Failure in this regard leads to some experts being very rarely used, so their parameters are of very little value to the model performance.
...
% Lory was heavily inspired by (\cite{muqeeth2023soft}). The authors of (\cite{muqeeth2023soft}) propose to, for each token, calculate weights for each expert and then calculate a weighted sum of experts. This new matrix is used to process the token in question. This approach works fine when it is used to fine tune models on downstream tasks, but the computational cost of recalculating new matrix in every step make this approach unfeasible to scale. 
\subsection{Lory}
...
% \[ O(h_p^j)= h_p^j + \sum_{i=1}^{i=N} e_i \cdot h_p^j  \cdot g(h_p^j,e_i)= h_p^j + \sum_{i=1}^{i=N} e_i \cdot h_p^j  \cdot \frac{exp( e_i \cdot h_p^j)}{\sum_{i=1}^{i=N} exp(e_i \cdot h_p^j)}\]
...
% We plan to implement and compare four models: Baseline MoE Transformer, Multi Head MoE, Lory for Expert Merging and the Combination of MH-MoE and Lory. We intend to compare them based on model perplexity, numbers of parameters needed to achieve similar pre-training accuracy, computational and memory cost, inference and training time. As our computational resources are quite limited, we do not plan to evaluate our models on any downstream task, and rather check their ability to predict next tokens from the dataset. We plan to baseline our models against a simple MoE transformer.
